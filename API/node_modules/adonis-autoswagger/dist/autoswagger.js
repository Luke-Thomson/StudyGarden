...
class AutoSwagger {
    ...
    scalar(url, proxyUrl = "https://proxy.scalar.com") {
        return `
      <!doctype html>
      <html>
        <head>
    ...
    async generate(adonisRoutes, options) {
        ...
        this.enumParser = new parsers_1.EnumParser();
        ...
        if (this.options.debug) {
            if (sourceFile !== "") {
                console.log(typeof customAnnotations !== "undefined" &&
                    ...
                    : `\x1b[33m✗ MISSING for ${action}\x1b[0m`, `${sourceFile} (${route.methods[0].toUpperCase()} ${route.pattern})`);
            }
        }
    ...
    async getSerializers() {
        const serializers = {};
        let p6 = path_1.default.join(this.options.appPath, "serializers");
    ...
    async getEnums() {
        let enums = {};
        const enumParser = new parsers_1.EnumParser();
        let p = path_1.default.join(this.options.appPath, "Types");
        let p6 = path_1.default.join(this.options.appPath, "types");
